...
func TestDispatcherImpl_Parse(t *testing.T) {
	d := NewDispatcher()
	for _, test := range tests {
		got := d.Parse(strings.NewReader(test.in))
		if got != test.want {
...
func TestReadlineImpl_Parse(t *testing.T) {
	r := NewReadline()
	for _, test := range tests {
		got := r.Parse(strings.NewReader(test.in))
		if got != test.want {
			t.Errorf("Parse(%q) == %q, want %q", test.in, got, test.want)
		}
	}
